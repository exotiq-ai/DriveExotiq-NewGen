...
export default function ApplicationsTable({
  ...
  const cities = useMemo(() => {
    const uniqueCities = new Set(
      applications.map((app) => app.city_of_interest),
    );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-graphite border border-metallic-silver/30 rounded-lg p-4">
          ...
          <div className="text-pure-white text-2xl font-bold mt-1">
            {stats.total}
          </div>
        </div>
        <div className="bg-graphite border border-performance-orange/30 rounded-lg p-4">
          ...
          <div className="text-performance-orange text-2xl font-bold mt-1">
            {stats.pending}
          </div>
        </div>
            ...
            <tbody className="divide-y divide-metallic-silver/30">
              {filteredApplications.length === 0 ? (
                <tr>
                  <td
                    colSpan={6}
                    className="px-6 py-8 text-center text-metallic-silver"
                  >
                ...
                filteredApplications.map((app) => (
                  <tr
                    key={app.id}
                    className="hover:bg-jet-grey/50 transition-colors"
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-metallic-silver">
                      {app.email}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gulf-blue">
                      {app.city_of_interest}
                    </td>
