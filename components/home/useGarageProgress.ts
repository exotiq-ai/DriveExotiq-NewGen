...
export default function useGarageProgress(
  ...
  useEffect(() => {
    const desktop = matchMedia(
      "(min-width: 1101px) and (min-height: 720px) and (pointer: fine)",
    );
  ...
  useEffect(() => {
    ...
    const layers = Array.from(
      root.querySelectorAll<HTMLElement>("[data-car-layer]"),
    );
    let frame = 0;
    let relevant = false;
    let disposed = false;
    ...
    function paint() {
      ...
      layers.forEach((layer, index) => {
        ...
        const incoming =
          index === 0 ? 1 : clamp((progress - (index / 3 - 0.065)) / 0.13);
    ...
    function schedule() {
      if (cinematic && relevant && !document.hidden && !frame)
  ...
  const select = useCallback(
    (index: number) => {
      ...
      if (mode.current && rootRef.current) {
        ...
        window.scrollTo({
          top: start + travel * HOLD_POINTS[index],
          behavior: "instant",
        });
      }
    },
