...
export default function HomeMotion() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      positions.forEach((top, index) => {
        if (top <= height * 0.46) current = index;
      });
      if (current !== selected) {
        selected = current;
        setActive(current);
      ...
      if (enabled) {
        ...
        hero.style.setProperty("--hero-scale", (1 + exit * 0.035).toFixed(4));
        const arrival = clamp((height - roadRect.top) / (height * 0.76));
        road.style.setProperty(
          "--road-inset",
          `${((1 - arrival) * 56).toFixed(2)}px`,
        );
        road.style.setProperty(
          "--road-radius",
          `${((1 - arrival) * 12).toFixed(2)}px`,
        );
        const words = clamp(
          (height * 0.83 - manifestoRect.top) / (height * 0.65),
        );
  ...
  return (
    <nav
      ...
      aria-label="Chapters in the drive"
    >
      ...
      <span className="home-chapter-track" aria-hidden="true">
        <span />
      </span>
    </nav>
  );
