...
export default function SiteHeader() {
  ...
  return <>
    ...
    <header className={`astra-header${scrolled?' is-scrolled':''}`}>
    ...
    <dialog id="astra-navigation" ref={dialog} className="astra-menu" aria-label="Site navigation" onClose={()=>{setOpen(false);trigger.current?.focus();}} onClick={event=>{if(event.target===dialog.current)close();}}>
      ...
      <nav aria-label="Mobile navigation">{links.map((link,index)=><Link href={link.href} onClick={close} key={link.href}><span className="astra-menu-number">0{index+1}</span>{link.label}<Arrow/></Link>)}<Link href="/sponsor" onClick={close}><span className="astra-menu-number">05</span>Partnerships<Arrow/></Link></nav>
