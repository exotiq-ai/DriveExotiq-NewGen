...
interface SmsConsentCheckboxesProps {
  register: (
    name: "smsTransactionalConsent" | "smsMarketingConsent",
...
export default function SmsConsentCheckboxes({
  ...
  const checkboxBg =
    variant === "dark" ? "bg-surface border-line" : "bg-white border-gray-300";
  const labelColor = variant === "dark" ? "text-ink-3" : "text-gray-500";

  return (
    <fieldset className="site-sms-consent min-w-0 space-y-4 border-0 p-0">
      <legend className={`mb-4 text-sm font-medium ${textColor}`}>
