...
Production deployment: `6aa31ef1759257c2b017be0b`. Live browser verified legacy analytics consent did not load Meta; selecting Marketing then loaded the real Meta script and Pixel-specific configuration, with the SDK initialized. The headless check did not show a PageView ingestion request, so Meta Events Manager receipt is not verified. No real Lead event was sent. Events Manager access remains needed for provider-side confirmation.
