...
if (kind === 'loop' || kind === 'play-once') {
  ff(['-i', work, '-c:v', 'libx264', '-profile:v', 'high', '-crf', '18', '-preset', 'slow', '-pix_fmt', 'yuv420p', '-an', '-movflags', '+faststart', path.join(VIDS, `${name}.mp4`)]);
  ff(['-i', work, '-vf', "scale='min(1280,iw)':-2", '-c:v', 'libx264', '-profile:v', 'high', '-crf', '21', '-preset', 'slow', '-pix_fmt', 'yuv420p', '-an', '-movflags', '+faststart', path.join(VIDS, `${name}.720.mp4`)]);
