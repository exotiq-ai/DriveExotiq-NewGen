...
test("the road film loads only on request, pauses background films, and releases on close", async ({
  page,
}) => {
  const filmRequests: string[] = [];
  const modalPosters: string[] = [];
  page.on("request", (request) => {
    if (request.url().includes("/media/v2/roadbook-"))
      filmRequests.push(request.url());
  });
  page.on("request", (request) => {
    if (new URL(request.url()).pathname === "/media/v2/hero-landscape-960.webp")
      modalPosters.push(request.url());
  });
  await page.goto("/");
  ...
  await expect
    .poll(() =>
      page
        .locator("video:not(.home-full-film)")
        .evaluateAll((videos) =>
...
test("a failed road film provides a useful fallback and Escape restores focus", async ({
  page,
}) => {
  ...
  await expect(
    dialog.getByRole("link", { name: "Explore the roadbook" }),
...
test("data-saving and reduced-motion visitors can explicitly choose the road film", async ({
  page,
}) => {
  ...
  expect(movies.every((url) => url.includes("/media/v2/roadbook-"))).toBe(true);
  await page.keyboard.press("Escape");
});

test("chapter navigation follows the real sections without trapping scroll", async ({
  page,
}, testInfo) => {
  test.skip(testInfo.project.name !== "desktop");
  await page.goto("/");
  const chapters = page.getByRole("navigation", {
    name: "Chapters in the drive",
  });
  await chapters.getByRole("link", { name: "4. The people" }).click();
  await expect(page).toHaveURL(/#the-people$/);
  await expect(
    chapters.getByRole("link", { name: "4. The people" }),
  ).toHaveAttribute("aria-current", "location");
  await expect(
    page.getByRole("heading", { name: /Some cars get collected/ }),
  ).toBeInViewport();
  await chapters.getByRole("link", { name: "5. Your invitation" }).click();
  await expect(
    chapters.getByRole("link", { name: "5. Your invitation" }),
  ).toHaveAttribute("aria-current", "location");
  await expect(
    page.getByRole("heading", { name: /See you out there/ }),
  ).toBeInViewport();
});
