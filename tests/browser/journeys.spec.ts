...
test('preview application validates, completes, and sends no production traffic', async ({ page }) => {
  ...
  await page.getByLabel('Full name', { exact: true }).fill('Preview Driver');
  await page.getByLabel('Email address', { exact: true }).fill('site-preview@example.com');
...
test('partnership form completes without promising a real submission', async ({ page }) => {
  ...
  await page.getByLabel('Your name', { exact: true }).fill('Preview Driver');
  await page.getByLabel('Email address', { exact: true }).fill('site-preview@example.com');
...
test('form failure preserves entered information and explains the error', async ({ page }) => {
  ...
  await expect(page.getByLabel('Email address', { exact: true })).toHaveValue('site-preview@example.com');
});
