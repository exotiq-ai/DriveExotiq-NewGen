...
const ART: Record<string, { src: string; alt: string; caption: string }> = {
  "the-car-sleeper-thesis": {
    src: "/media/s8-desert-vista.webp",
    alt: "The Audi S8 at a golden-hour desert overlook",
    caption: "The Audi S8. / Tortilla Flats, from the Drive Exotiq archive.",
  },
  "tour-denver": {
    src: "/media/s8-alpine-drive.webp",
    alt: "An Audi S8 on a winding alpine road in Colorado",
    caption:
      "A reason to leave the city early. / Colorado, from the Drive Exotiq archive.",
  },
  "tour-miami": {
    src: "/media/r8-rain-detail.webp",
    alt: "A close look at the rain-beaded hood of an Audi R8",
    caption:
      "An eye for the details. / Audi R8 photographed in Telluride, from the Drive Exotiq archive.",
  },
};
...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      ...
      type: "article",
      images: [
        { url: art?.src || "/og-image.jpg", alt: art?.alt || "Drive Exotiq" },
      ],
    },
    twitter: {
  ...
  return (
    <>
      ...
      <main id="main-content" className="ed-page">
        ...
        />
        <article>
          <header className="ed-wrap ed-article-head">
            <p className="ed-kicker">The Journal / {post.category}</p>
            <h1 className="ed-title">{post.title}</h1>
            {post.dek && <p className="ed-lead">{post.dek}</p>}
          ...
          {art && (
            <figure>
              <div className="ed-photo ed-article-photo">
                <Image
                  src={art.src}
                  alt={art.alt}
                  fill
